...
import { useBoolean, useHookTableV1 as useHookTable } from '@sa/hooks';
...
export function useTable<A extends NaiveUI.TableApiFn>(config: NaiveUI.NaiveTableConfig<A>) {
  ...
  // * v1 的表格行数据会追加 index 字段，这里统一行类型，避免 columns/data 推断断链
  type Row = NaiveUI.TableDataWithIndex<GetTableData<A>>;
  ...
  const {
    ...
    updateSearchParams,
    resetSearchParams
  } = useHookTable<A, GetTableData<A>, TableColumn<Row>>({
    apiFn,
    apiParams,
    ...
    getColumns: (cols, checks) => {
      type Col = TableColumn<Row>;
      const columnMap = new Map<string, Col>();
      ...
      const filteredColumns = checks.filter(item => item.checked).map(check => columnMap.get(check.key) as Col);
  ...
  /**
   * * v1 兼容层：对外导出 naive-ui 官方 columns 类型
   * ! 目的：当 Row 类型推断为 any 时，NaiveUI.TableColumn<any> 的 key 可能包含 symbol，导致 NDataTable :columns 报错
   * * 这里只做类型边界适配，不改变运行时列结构
   */
  const naiveColumns = computed(() => columns.value as DataTableColumns<Row>);
...
export function wrapApiFn(fn: (params: any) => Promise<any>) {
  return async (params: any) => {
    ...
    let newData: Api.Common.PaginatingQueryRecord = data;
    if (!error) {
      const {
        data: records = [],
        total = 0,
    ...
    return { data: newData, error, response };
  };
}
